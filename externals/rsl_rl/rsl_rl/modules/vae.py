...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        # MLP Encoder
        encoder_layers = []
        encoder_layers.append(nn.Linear(num_history_input, encoder_hidden_dims[0]))
        encoder_layers.append(activation)
        for l in range(len(encoder_hidden_dims)):
            if l == len(encoder_hidden_dims) - 1:
                encoder_layers.append(
                    nn.Linear(encoder_hidden_dims[l], num_latent_dims * 2 + num_explicit_dims * 2)
                )  # output latent + base_lin_vel(estimated)
                encoder_layers.append(activation)
            else:
