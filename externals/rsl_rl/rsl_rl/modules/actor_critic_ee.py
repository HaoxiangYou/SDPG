...
class ActorCriticEE(nn.Module):
        ...
        **kwargs,
        ...
        for l in range(len(estimator_hidden_dims)):
            if l == len(estimator_hidden_dims) - 1:
                estimator_layers.append(nn.Linear(estimator_hidden_dims[l], num_estimator_labels))
            else:
    ...
    def update_distribution(self, estimator_features):
        ...
        mean = self.actor(torch.cat((estimator_features, estimator_output), dim=-1))
    ...
    def act_inference(self, estimator_features):
        estimator_output = self.estimator(estimator_features)
        actions_mean = self.actor(torch.cat((estimator_features, estimator_output), dim=-1))
        return actions_mean
