...
class Walker(MujocoEnv):
    """Walker environment (MJX). Mirrors the genesis backend walker task."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            xml_path=os.path.join(os.path.dirname(__file__), "../../assets/mujoco/walker.xml"),
    ...
    def init_task(self) -> None:
        # qpos/qvel layout follows the MJCF joint order: [x, z(height), y(angle)].
        # NOTE: the genesis walker orders the root DOFs [x, angle, height]; the
        # observation/reward code below permutes accordingly so the observation
        # layout matches the genesis backend exactly.
        self._root_dof_idx = [0, 1, 2]  # rootx (x slide), rootz (z slide), rooty (rotation)
        self._motor_dof_idx = [3, 4, 5, 6, 7, 8]  # right hip/knee/ankle, left hip/knee/ankle
    ...
    def compute_observations(self, states: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Genesis walker observation layout: [angle, height, motor_pos,
        # x_vel, angle_vel, height_vel, motor_vel] (root order [x, angle, height]
        # there; internally ours is [x, height, angle], hence the permutations).
        privileged_observations = torch.cat(
            [
                robot_states["root_joints_pos"][:, [2, 1]],
                robot_states["motor_joints_pos"],
                robot_states["root_joints_vel"][:, [0, 2, 1]],
    ...
    def compute_termination(self, states: Dict[str, Any]) -> torch.Tensor:
        ...
        if self._early_termination:
            ...
            angle = robot_states["root_joints_pos"][:, 2]
