...
Go2 = scene.add_entity(gs.morphs.URDF(file="urdf/go2/urdf/go2.urdf", pos=[0.0, 0.0, 0.42], quat=[1.0, 0.0, 0.0, 0.0]))

# Go2_test = scene.add_entity(gs.morphs.MJCF(file='go2_hardware/mujoco_menagerie/unitree_go2/go2.xml'
# , pos=[0.0, 1.0, 0.42], quat=[1.0, 0.0, 0.0, 0.0]))
...
for i in range(500):
    torques = (
        kp * (initial_position - Go2.get_dofs_position(motors_dof_idx).detach().numpy())
        - kd * Go2.get_dofs_velocity(motors_dof_idx).detach().numpy()
    )
    Go2.control_dofs_force(torques, motors_dof_idx)
    # Go2.control_dofs_position(initial_position, motors_dof_idx)
    # Go2_test.control_dofs_position(initial_position, motors_dof_idx)
    scene.step()

# Analyze the robot
robot_mass = sum(link.inertial_mass for link in Go2.links)
print(f"Calculated total robot mass: {robot_mass:.2f} kg")

print("\n--- Link Inertial Properties ---")
for link in Go2.links:
    print(f"\nLink: {link.name}")
...
all_pos_lower, all_pos_upper = Go2.get_dofs_limit()
all_force_lower, all_force_upper = Go2.get_dofs_force_range()
all_armature = Go2.get_dofs_armature()
all_kp = Go2.get_dofs_kp()
all_kv = Go2.get_dofs_kv()
all_damping = Go2.get_dofs_damping()
all_stiffness = Go2.get_dofs_stiffness()
all_invweight = Go2.get_dofs_invweight()

print("\n--- Joint Properties ---")
for name in jnt_names:
    joint = Go2.get_joint(name)
